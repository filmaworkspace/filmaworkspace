...
export type FringePeriod = "day" | "week" | "month" | "year";
...
/** Duración en días de cada periodo de fringe, para convertir entre la Unidad de una línea y el periodo configurado en un fringe (ver `computeFringeAmountForLine`). Mes y año en su duración media, no la de un mes/año concreto. */
const FRINGE_PERIOD_DAYS: Record<FringePeriod, number> = { day: 1, week: 7, month: 30.4375, year: 365.25 };

/**
 * Reconoce si el nombre de una Unidad de línea (p.ej. "Día", "semanas",
 * "Month") corresponde a uno de los periodos de fringe (día/semana/mes/año),
 * para poder convertir entre la Unidad de la línea y el periodo del fringe.
 * Devuelve null si no es un periodo de tiempo reconocible (p.ej. "Persona",
 * "Fijo", "%"): en ese caso no hay conversión posible.
 */
function matchFringePeriod(unit: string | undefined | null): FringePeriod | null {
  const u = (unit || "").trim().toLowerCase();
  if (["día", "dia", "días", "dias", "day", "days"].includes(u)) return "day";
  if (["semana", "semanas", "week", "weeks"].includes(u)) return "week";
  if (["mes", "meses", "month", "months"].includes(u)) return "month";
  if (["año", "ano", "años", "anos", "year", "years"].includes(u)) return "year";
  return null;
}
 ...
 *  - "fixed_period": importe fijo por periodo (día/semana/mes/año), con tope
 *    opcional — modela una cotización topada, p.ej. la Seguridad Social
 *    española: por debajo del tope se cotiza como porcentaje (otro fringe de
 *    tipo "percent"), pero a partir del tope la cotización se congela en un
 *    importe fijo por periodo, por mucho que se cobre por encima. Por eso el
 *    tope no limita el propio importe fijo (sería absurdo, siempre sería
 *    igual al importe): compara la Tarifa de la línea (lo que cobra esa
 *    persona por periodo) contra el tope — si la iguala o supera, se aplica
 *    el importe fijo por cada periodo que dure la línea; si no lo alcanza,
 *    este fringe no genera nada (lo cubre el fringe porcentual). La Tarifa y
 *    el tope se comparan en los mismos términos de tiempo que el periodo del
 *    fringe, convirtiendo la Unidad de la línea si es un periodo de tiempo
 *    reconocible (Día/Semana/Mes/Año) distinto — ver `computeFringeAmountForLine`.
 *    El tope se aplica por línea: no hay todavía un modelo de "misma
 *    persona" entre líneas distintas para acumularlo entre ellas (eso
 *    requeriría un sistema de contactos, fuera de alcance por ahora).
...
export interface BudgetingExportConfig {
  /** Los reportes del PDF, cada uno se puede incluir o no por separado (ver componente Reportes). */
  includeCoverSheet: boolean;
  includeTopSheet: boolean;
  includeDetail: boolean;
  /** Reporte aparte, siempre el último del PDF si está activo: mismo formato Capítulo→Cuenta que el Top Sheet, pero con lo que genera cada uno en Cargas Sociales (sea cual sea el alcance de cada fringe), no con el presupuesto normal. */
  includeFringesReport: boolean;
  ...
  /** Si es true, las Cuentas que no tengan ninguna línea añadida (de texto, normal o subtotal) no se dibujan en el Detalle del PDF, tengan o no total (a diferencia de `hideZeroTotalSubchapters`, que mira el importe, esto mira si hay algo añadido). El Capítulo en sí nunca se oculta por esto: sigue apareciendo con su nombre y su total (0€ si ninguna de sus Cuentas tiene líneas), solo se ocultan las Cuentas vacías de dentro. */
  hideEmptySubchapters: boolean;
  /** Idioma de los textos fijos del PDF (ver PdfLanguage): lo escrito en el presupuesto no se traduce. */
  pdfLanguage: PdfLanguage;
  /** Columnas de línea que se pueden mostrar/ocultar en el PDF (tabla de Detalle) y el Excel: `qty`/`mult`/`rate`/`subtotal`/`total` son las que ya salían siempre, fijas — ahora también se pueden quitar, igual que `unit` (que ya era opcional). Todas empiezan en `true`, para que el comportamiento por defecto sea exactamente el de siempre. `notes`/`tags` siguen empezando en `false`, como hasta ahora. `total` solo afecta al PDF (la columna de las filas de cierre — TOTAL de Cuenta/Capítulo/presupuesto): el Excel no tiene esa distinción, es una lista plana de líneas, así que su única columna de importe se rige por `subtotal`. */
  fields: {
    qty: boolean;
    unit: boolean;
    mult: boolean;
...
export const DEFAULT_EXPORT_CONFIG: BudgetingExportConfig = {
  ...
  includeDetail: true,
  includeFringesReport: false,
  pageBreakPerChapter: false,
  pdfFontSize: "normal",
  hideZeroTotalSubchapters: false,
  hideEmptySubchapters: false,
  pdfLanguage: "es",
  fields: { qty: true, unit: true, mult: true, rate: true, subtotal: true, total: true, notes: false, tags: false },
};
...
export function normalizeExportConfig(raw: (Partial<BudgetingExportConfig> & { coverSheet?: boolean }) | null | undefined): BudgetingExportConfig {
  ...
  return {
    ...
    includeFringesReport: raw.includeFringesReport ?? DEFAULT_EXPORT_CONFIG.includeFringesReport,
    ...
    hideEmptySubchapters: raw.hideEmptySubchapters ?? DEFAULT_EXPORT_CONFIG.hideEmptySubchapters,
...
export interface BudgetingProjectInfo {
  ...
  writer?: string;
  /** Director de producción (Line Producer / UPM). */
  lineProducer?: string;
  preparedBy?: string;
  ...
  startDate?: string;
  endDate?: string;
  /** Nº de días de rodaje (texto libre: a veces se anota "24 + 3 viaje", no siempre un número solo); es el único de los tres que es un recuento de días, no una fecha. */
  shootDays?: string;
  /** Fecha de inicio de preproducción y de postproducción, por separado (antes un único campo "Postproducción" en días). */
...
/** Importe que genera un fringe sobre una línea concreta (ver BudgetingFringe para la lógica del tope). */
export function computeFringeAmountForLine(fringe: BudgetingFringe, line: { total: number; units: number; rate?: number; unit?: string | null }): number {
  if (fringe.type === "percent") {
    return Math.round((line.total || 0) * ((fringe.percent || 0) / 100) * 100) / 100;
  }
  const amount = fringe.amount || 0;
  const fringePeriod = fringe.period || "month";
  const linePeriod = matchFringePeriod(line.unit);
  // Nº de periodos del fringe que representan las `units` de la línea (p.ej.
  // 4 semanas -> ~0,92 meses si el fringe es mensual); sin conversión
  // reconocible, se asume 1:1 (las `units` ya son periodos del fringe).
  const periods = linePeriod ? (line.units || 0) * (FRINGE_PERIOD_DAYS[linePeriod] / FRINGE_PERIOD_DAYS[fringePeriod]) : (line.units || 0);
  if (fringe.capAmount == null) {
    return Math.round(amount * periods * 100) / 100;
  }
  // Tarifa de la línea (por una Unidad de la línea) llevada a "por un periodo del fringe", para compararla contra el tope en los mismos términos.
  const ratePerFringePeriod = linePeriod ? (line.rate || 0) * (FRINGE_PERIOD_DAYS[fringePeriod] / FRINGE_PERIOD_DAYS[linePeriod]) : (line.rate || 0);
  if (ratePerFringePeriod < fringe.capAmount) return 0;
  return Math.round(amount * periods * 100) / 100;
}
