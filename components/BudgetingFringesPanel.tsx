...
export default function BudgetingFringesPanel({ draftId }: { draftId: string }) {
  const { user } = useUser();
    ...
    <div className="space-y-6">
      ...
      {modalItem !== undefined && (
        <div
          ...
          onMouseDown={(e) => { if (e.target === e.currentTarget) setModalItem(undefined); }}
        >
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-slate-100">
              <h3 className="text-sm font-semibold text-slate-900">{modalItem ? "Editar carga social" : "Nueva carga social"}</h3>
              ...
              {form.type === "percent" ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <Segmented options={(["day", "week", "month", "year"] as FringePeriod[]).map((p) => ({ value: p, label: FRINGE_PERIOD_LABELS[p] }))} value={form.period} onChange={(period) => setForm((f) => ({ ...f, period }))} />
                  </div>
                  <p className="text-[11px] text-slate-400">
                    Sin tope: importe × Cantidad de la línea. Con tope: solo se aplica si la Tarifa de la línea iguala o supera el tope
                    (si no, esta carga social no genera nada); la Unidad de la línea se convierte sola si es un periodo distinto al de aquí.
                  </p>
                </>
              )}
      ...
      {folderEditTarget && (
        <div
          className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[70] flex items-center justify-center p-4"
          onMouseDown={(e) => { if (e.target === e.currentTarget) setFolderEditTarget(null); }}
        >
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            ...
            <div className="px-5 py-4 space-y-3.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1.5">Todas las cargas sociales de esta carpeta se funden en una sola línea de presupuesto con este código.</p>
              </div>
            </div>
            <div className="px-5 py-3 border-t border-slate-100 flex gap-2">
      ...
      {deleteTarget && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-5">
            <h3 className="text-sm font-semibold text-slate-900 mb-1.5">Borrar carga social "{deleteTarget.label}"</h3>
