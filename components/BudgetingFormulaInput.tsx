...
export default function BudgetingFormulaInput({ value, onChange, onBlur, onKeyDown, globals, className, autoFocus, title }: Props) {
      ...
      {open && suggestions.length > 0 && (
        <div className="absolute z-30 top-full left-0 mt-0.5 w-52 bg-white border border-slate-200 rounded-lg shadow-lg py-1 max-h-40 overflow-y-auto">
          {suggestions.map((g) => (
            ...
            >
              <span className="text-[11px] font-mono flex-shrink-0" style={{ color: "#8DA7BE" }}>{g.code}</span>
