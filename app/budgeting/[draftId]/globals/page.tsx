    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {scenarios.map((sc) => (
          editingId === sc.id ? (
            <input key={sc.id} autoFocus value={editLabel} onChange={(e) => setEditLabel(e.target.value)}
              ...
              className="text-xs px-2.5 py-1 border border-[#414E82] rounded-full focus:outline-none w-28" />
          ) : (
            <span key={sc.id} className="inline-flex items-center gap-1 text-xs pl-2.5 pr-1.5 py-1 rounded-full border border-slate-200 text-slate-600">
              <button onClick={() => { setEditingId(sc.id); setEditLabel(sc.label); }} className="hover:text-[#414E82]">{sc.label}</button>
        ...
        <input
          ...
          className="text-xs px-2.5 py-1 rounded-full border border-dashed border-slate-300 focus:outline-none focus:border-[#414E82] w-32"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-5 py-2.5 hover:bg-slate-50 group">
      <div className="flex items-center gap-2.5 min-w-0">
        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded flex-shrink-0" style={{ background: "#414E821a", color: "#1D201F" }}>{g.code}</span>
        <span className="text-sm font-medium text-slate-900 truncate">{g.label}</span>
        {!isPlainNumber(g.value) && <span title="Fórmula"><Sigma size={11} className="text-[#414E82] flex-shrink-0" /></span>}
