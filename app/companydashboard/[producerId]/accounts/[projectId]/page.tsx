...
export default function CompanyAccountsPage() {
  ...
  const currentIndex = selectedInvoice
    ? filteredInvoices.findIndex((i) => i.id === selectedInvoice.id)
    : -1;
  ...
  return (
    <div className={`min-h-screen bg-white ${inter.className}`}>
      ...
      {/* ── Fixed top bar: breadcrumb + sub-nav ─────────────────────────────── */}
      ...
      {/* Offset for fixed top bar (original navbar 64px + breadcrumb ~37px + subnav ~36px) */}
      <div className="mt-[137px]">
              ...
              {showFilterDropdown && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 py-1 min-w-[170px]">
                  {FILTER_OPTIONS.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => { setStatusFilter(option.value); setShowFilterDropdown(false); }}
        ...
        <div className="flex">
          ...
          {/* Invoice table */}
          <div className={`flex-1 transition-all ${selectedInvoice ? "mr-[50%]" : ""} p-4`}>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-slate-50 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 text-slate-500 text-xs mb-1">
                        <Calendar size={12} /> Fecha factura
                      </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-1.5 text-xs">
                    {[
                      ...
                      <div key={r.label} className="flex justify-between">
                        ...
                        <span className={`font-mono font-medium ${r.value < 0 ? "text-red-600" : "text-slate-900"}`}>
                      ...
                      {accountingForm.entryNumber.trim() && (
                        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">
                            Preview asiento — {accountingForm.entryNumber}
                          </p>
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="text-slate-400 text-[10px]">
                                <th className="text-left pb-1.5 font-medium">Cuenta</th>
                                <th className="text-right pb-1.5 font-medium">Debe</th>
                                <th className="text-right pb-1.5 font-medium">Haber</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                              {selectedInvoice.items.map((item: any, i: number) => (
                                <tr key={i}>
                                  <td className="py-1 font-mono text-slate-600">{item.subAccountCode} – {item.description}</td>
                                  <td className="py-1 text-right font-mono font-semibold text-slate-900">{formatCurrency(item.baseAmount)}</td>
                                  <td />
                                </tr>
                              ))}
                              {selectedInvoice.vatAmount > 0 && (
                                <tr>
                                  <td className="py-1 font-mono text-slate-600">472 – H.P. IVA soportado</td>
                                  <td className="py-1 text-right font-mono font-semibold text-slate-900">{formatCurrency(selectedInvoice.vatAmount)}</td>
                                  <td />
                                </tr>
                              )}
                              {selectedInvoice.irpfAmount < 0 && (
                                <tr>
                                  <td className="py-1 font-mono text-slate-600">473 – H.P. retenciones practicadas</td>
                                  <td />
                                  <td className="py-1 text-right font-mono font-semibold text-red-600">{formatCurrency(Math.abs(selectedInvoice.irpfAmount))}</td>
                                </tr>
                              )}
                              <tr>
                                <td className="py-1 font-mono text-slate-600">400 – Proveedores</td>
                                <td />
                                <td className="py-1 text-right font-mono font-semibold text-red-600">
                                  {formatCurrency(
                                    selectedInvoice.totalAmount +
                                    (selectedInvoice.irpfAmount < 0 ? Math.abs(selectedInvoice.irpfAmount) : 0)
                                  )}
                                </td>
                              </tr>
                            </tbody>
