...
function Field({
  ...
  const masked = sensitive && !revealed && display
    ? String(value).replace(/./g, "•").slice(0, 16)
    : undefined;
...
export default function CrewMemberPage() {
  ...
  // ── Load ───────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (userId && projectId && memberId) loadMember();
  }, [userId, projectId, memberId]);
  ...
  const saveEdit = async () => {
    ...
    setSaving(true);
    try {
      const updated = { ...member, ...draft, updatedAt: Timestamp.now(), updatedBy: userId };
      await updateDoc(doc(db, `projects/${projectId}/crew`, memberId), updated);
      setMember(updated as CrewMember);
  ...
  const setField = (key: keyof CrewMember, value: any) =>
    setDraft((d) => ({ ...d, [key]: value }));

  const get = (key: keyof CrewMember): any =>
    editingSection ? (draft[key] ?? member?.[key]) : member?.[key];
  ...
  const handleStatusChange = async (status: CrewMember["status"]) => {
    ...
    await updateDoc(doc(db, `projects/${projectId}/crew`, memberId), {
      status, updatedAt: Timestamp.now(), updatedBy: userId,
    });
  ...
  const handleSendForm = async () => {
    ...
    setSendingForm(true);
    try {
      // Aquí iría la llamada a tu función de envío de email / Cloud Function
      // Por ahora marcamos en Firestore que se envió
      await updateDoc(doc(db, `projects/${projectId}/crew`, memberId), {
  ...
  const completeness = (() => {
    if (!member) return 0;
    const fields = [
      member.phone, member.email, member.address, member.dni,
      member.socialSecurityNumber, member.birthDate, member.nationality,
      member.grossSalary, member.bankAccount,
    ];
        ...
        <div className="bg-white border-b border-slate-200 px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-24 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm">
              <Link
                href={`/project/${projectId}/team/crew`}
              ...
              <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg">
                <div className="w-20 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${completeness}%`,
                      backgroundColor: completeness >= 80 ? "#6BA319" : completeness >= 50 ? "#F59E0B" : "#EF4444",
                    }}
                  />
                </div>
                    ...
                    {(Object.entries(STATUS_MAP) as [CrewMember["status"], typeof STATUS_MAP.active][]).map(([key, conf]) => (
                      <button
                        ...
                        className={`w-full px-3 py-2.5 text-left text-xs flex items-center gap-2 transition-colors ${
                          member.status === key ? "bg-slate-50 font-semibold" : "text-slate-700 hover:bg-slate-50"
                        }`}
                      >
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col items-center text-center">
              ...
              {/* Cast extra */}
              {member.section === "cast" && (member.character || member.sessions) && (
            ...
            <div className={`border rounded-2xl p-4 ${member.formSentAt ? "bg-emerald-50 border-emerald-200" : "bg-white border-slate-200"}`}>
              <div className="flex items-center gap-3">
                ...
                {member.formSentAt && (
                  <button
                    onClick={() => setShowFormModal(true)}
                    className="text-xs text-emerald-600 hover:text-emerald-800 flex-shrink-0"
                    title="Reenviar"
                  >
                    <RefreshCw size={13} />
                  </button>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <SectionCard title="Contrato & Remuneración" editing={isEdit("contract")} onEdit={() => startEdit("contract")}>
              <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <Field label="Salario bruto (€)" value={get("grossSalary")}  editing={isEdit("contract")} type="number" onChange={(v) => setField("grossSalary", Number(v))}  placeholder="0.00" />
                ...
                <Field label="Nº cuenta corriente" value={get("bankAccount")} editing={isEdit("contract")} onChange={(v) => setField("bankAccount", v)} placeholder="ES00 0000 0000 0000 0000 0000" sensitive />
              </div>
              {isEdit("contract") && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <EditActions saving={saving} onSave={saveEdit} onCancel={cancelEdit} />
            ...
            {/* Notes — si no están en la sidebar */}
            {!member.notes && !isEdit("notes") && (
              <button
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-slate-500 mb-5">
              Se enviará un enlace personalizado a <span className="font-semibold text-slate-700">{member.email}</span> para que el miembro complete su ficha: datos personales, DNI, cuenta bancaria y demás información de alta.
            </p>
...
function EditActions({
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <button
        onClick={onCancel}
        className="px-3 py-2 text-xs text-slate-600 hover:text-slate-900 border border-slate-200 rounded-lg hover:bg-white transition-colors"
      >
        Cancelar
      </button>
      <button
