...
export default function NewInvoicePage() {
  ...
  }, [id, permissions, permissionsLoading, preselectedPoId]);

  const loadPOItemsInvoiced = useCallback(async () => {
  ...
  const updateAvailableWithCurrentItems = useCallback(() => {
  ...
  const calculatePOStats = useCallback(async () => {
    if (!selectedPO) return;
  ...
  }, [id, selectedPO, totals.totalAmount]);
  ...
  const startReplacement = useCallback((pd: PendingDocument) => {
  ...
  }, [linkToId, pendingDocuments, linkToProcessed, replaceMode, startReplacement]);
