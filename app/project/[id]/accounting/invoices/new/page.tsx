...
export default function NewInvoicePage() {
  ...
  useBodyScrollLock(
    showPOModal || showAccountModal || showSupplierModal || showCreateSupplierModal ||
    showPOItemsModal || showReplaceModal || showEpisodeModal
  );
  ...
  useEffect(() => {
    if (id) {
      const savedDrafts = localStorage.getItem(`invoice_drafts_${id}`);
      if (savedDrafts) {
        try {
  ...
  const saveDraft = () => {
    const hasContent = formData.supplier || formData.description || 
    ...
    const newDraft: Draft = {
      id: `draft_${Date.now()}`,
      name: formData.supplierName || formData.description || `Borrador ${drafts.length + 1}`,
    ...
    localStorage.setItem(`invoice_drafts_${id}`, JSON.stringify(updatedDrafts));
  ...
  const createQuickSupplier = async () => {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, supplier: docRef.id, supplierName: newSupplierData.fiscalName.trim() }));
      setTouched((p) => ({ ...p, supplier: true }));

      // Añadir a la lista local
      setSuppliers((prev) => [...prev, { id: docRef.id, ...newSupplier } as Supplier]);
  ...
  return (
    <div className={cx("min-h-screen bg-white", inter.className)}>
        ...
        <div className="px-24 pt-10 pb-6">
          <div className="relative flex items-center justify-center">
            ...
            <div className="flex items-center gap-4">
              <DocIcon size={22} style={{ color: "#2F52E0" }} />
      ...
      {/* PO Modal */}
      {showPOModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => { setShowPOModal(false); setPOSearch(""); }}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden border border-slate-200" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
      ...
      {showPOItemsModal && selectedPO && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowPOItemsModal(false)}>
            ...
            <div className="p-6">
              <div className="flex gap-2 mb-4">
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-colors flex items-center gap-2 text-sm font-medium"
                >
                  <Plus size={16} />
                  Nuevo
                </button>
              ...
              <div className="max-h-80 overflow-y-auto space-y-2">
                {suppliers.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-slate-500 mb-3">No hay proveedores registrados</p>
                    <button
                      onClick={() => setShowCreateSupplierModal(true)}
      ...
      {showCreateSupplierModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4" onClick={() => { setShowCreateSupplierModal(false); setNewSupplierData({ fiscalName: "", taxId: "" }); }}>
      ...
      {showEpisodeModal && episodeItemIndex !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowEpisodeModal(false)}>
