...
export default function DocumentCenterPage() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return f;
  }, [invoices, searchTerm, supplierFilter, statusFilter, dateRange]);
  ...
  const downloadSingle = async (invoice: Invoice) => {
    try {
      ...
      if (bytes) triggerDownload(bytes, `Expediente_${invoice.displayNumber}.pdf`);
    } catch (e) { console.error(e); }
  ...
  const downloadMultiple = async (list: Invoice[]) => {
    ...
    try {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (bytes) triggerDownload(bytes, `Expediente_${list[i].displayNumber}.pdf`);
      ...
      <main className="px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-24 py-8 space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <FilterDropdown open={showSupplierDD}
              onToggle={() => { setShowSupplierDD(!showSupplierDD); setShowStatusDD(false); }}
              label={supplierFilter === "all" ? "Todos los proveedores" : (suppliers.find(s => s[0] === supplierFilter)?.[1] || "Proveedor")}>
              <DDOpt active={supplierFilter === "all"} onClick={() => { setSupplierFilter("all"); setShowSupplierDD(false); }}>Todos los proveedores</DDOpt>
              {suppliers.map(([sid, sname]) => (
                <DDOpt key={sid} active={supplierFilter === sid} onClick={() => { setSupplierFilter(sid); setShowSupplierDD(false); }}>{sname}</DDOpt>
              ))}
            </FilterDropdown>

            <FilterDropdown open={showStatusDD}
              onToggle={() => { setShowStatusDD(!showStatusDD); setShowSupplierDD(false); }}
            ...
            {hasFilters && (
              <button onClick={() => { setSearchTerm(""); setSupplierFilter("all"); setStatusFilter("all"); setDateRange({ from: "", to: "" }); }}
        ...
        {supplierFilter === "all" && suppliers.length > 1 && (
          <div>
