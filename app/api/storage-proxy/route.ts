...
// pdf-merger-js ya no se necesita — se elimina
...
// ─── THE FIX ────────────────────────────────────────────────────────────────
// Replaces mergePdfsWithMerger. Normalizes the invoice PDF before merging
// to resolve compressed Object Streams (ObjStm) that cause blank pages
// when pdf-lib tries to copy multi-page PDFs generated by external tools.
async function mergePdfs(bannerBytes: Uint8Array, docBytes: Uint8Array): Promise<Uint8Array> {
  // Step 1: Normalize the invoice PDF.
  // Load → save with useObjectStreams:false → reload.
  // This forces pdf-lib to decompress all ObjStm entries and re-serialize
  // every page's content stream as a direct object. Without this, copyPages()
  // on a multi-page ObjStm PDF produces pages with dangling content references → blank.
  ...
  // Step 2: Load the banner (always clean — generated by us).
  const bannerDoc = await PDFDocument.load(bannerBytes, { ignoreEncryption: true });

  // Step 3: Merge into a new document: banner first, then all invoice pages.
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // If no expense data, just return the raw file
    if (!expenseRaw) {
    ...
    // Parse expense data and generate banner
